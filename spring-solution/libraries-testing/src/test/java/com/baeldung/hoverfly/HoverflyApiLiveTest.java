...
public class HoverflyApiLiveTest {
    ...
    private static final SimulationSource source = dsl(service("http://www.example.com").get("/api/courses/1").willReturn(success().body(jsonWithSingleQuotes("{'id':'1','name':'HCI'}")))
    ...
    public void givenGetCourseById_whenRequestSimulated_thenAPICalledSuccessfully() throws URISyntaxException {
        final ResponseEntity<String> courseResponse = restTemplate.getForEntity("http://www.example.com/api/courses/1", String.class);
    ...
    public void givenPostCourse_whenDelayInRequest_thenResponseIsDelayed() throws URISyntaxException {
        ...
        stopWatch.start();
        final ResponseEntity<Void> postResponse = restTemplate.postForEntity("http://www.example.com/api/courses", null, Void.class);
        stopWatch.stop();
    ...
    public void givenGetStudent_whenRequestMatcher_thenAPICalledSuccessfully() throws URISyntaxException {
        final ResponseEntity<Void> courseResponse = restTemplate.getForEntity("http://www.example.com/api/student?page=3", Void.class);
    ...
    public void givenPostStudent_whenBodyRequestMatcherJson_thenResponseContainsEqualJson() throws URISyntaxException {
        final ResponseEntity<Void> postResponse = restTemplate.postForEntity("http://www.example.com/api/student", "{\"id\":\"1\",\"name\":\"Joe\"}", Void.class);
    ...
    public void givenPutStudent_whenJsonPathMatcher_thenRequestJsonContainsElementInPath() throws URISyntaxException {
        RequestEntity<String> putRequest = RequestEntity.put(new URI("http://www.example.com/api/student/1")).body("{\"id\":\"1\",\"name\":\"Trevor\"}");
    ...
    public void givenPostStudent_whenBodyRequestMatcherXml_thenResponseContainsEqualXml() throws URISyntaxException {
        final ResponseEntity<Void> postResponse = restTemplate.postForEntity("http://www.example.com/api/student", "<student><id>2</id><name>John</name></student>", Void.class);
    ...
    public void givenPutStudent_whenXPathMatcher_thenRequestXmlContainsElementInXPath() throws URISyntaxException {
        RequestEntity<String> putRequest = RequestEntity.put(new URI("http://www.example.com/api/student/2")).body("<?xml version=\"1.0\" encoding=\"UTF-8\" ?>" + "<student><id>2</id><name>Monica</name></student>");
