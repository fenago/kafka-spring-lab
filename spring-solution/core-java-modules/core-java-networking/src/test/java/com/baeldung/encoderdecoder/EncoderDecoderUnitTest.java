...
public class EncoderDecoderUnitTest {
    ...
    public void givenURL_whenAnalyze_thenCorrect() throws Exception {
        ...
        Assert.assertThat(url.getHost(), is("www.example.com"));
    ...
    public void givenRequestParam_whenUTF8Scheme_thenEncode() throws Exception {
        ...
        String encodedURL = requestParams.keySet().stream().map(key -> key + "=" + encodeValue(requestParams.get(key))).collect(joining("&", "http://www.example.com?", ""));
    ...
    public void givenPathAndRequestParam_whenUTF8Scheme_thenEncode() throws Exception {
        ...
        String encodedURL = requestParams.keySet().stream().map(key -> key + "=" + encodeValue(requestParams.get(key))).collect(joining("&", "http://www.example.com/" + encodePath(path) + "?", ""));
